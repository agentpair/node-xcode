...
pbxProject.prototype.removeLocale = function(localeName) {
    var k, k2,
        row, row2, file,
        localizedStringsDirname = localeName+".lproj",
        resourcesGroupChildren = this.pbxGroupByName("Resources").children,
        localizableStringsGroup = this.pbxVariantGroupsByName("Localizable.strings");

    for(k in resourcesGroupChildren) {
        if(resourcesGroupChildren[k].comment===localizedStringsDirname) {
            resourcesGroupChildren.splice(k, 1);
            break;
        }
    }
    this.removePbxGroupByName(localizedStringsDirname);

    for(k in localizableStringsGroup) {
        if(localizableStringsGroup.hasOwnProperty(k)) {
            row = localizableStringsGroup[k];
            for(k2 in row.children) {
                if(row.children.hasOwnProperty(k2)) {
                    row2 = row.children[k2];
                    if(row2.comment===localeName) {
                        file = new pbxFile(localeName, {lastType: "text.plist.strings"});
                        file.path = "Localizable.strings";
                        this.removeFromPbxFileReferenceSectionUsingExactMatch(file);
                    }
                }
            }
        }
...
pbxProject.prototype.removeFromPbxFileReferenceSectionUsingExactMatch = function(file) {
    var k, item,
        section = this.pbxFileReferenceSection();

    for(k in section) {
        if(section.hasOwnProperty(k)) {
            item = section[k];
            if(item.name===file.basename && item.path===file.path) {
                console.log("Found file - exact match");
                delete section[k];
                if(k+"_comment" in section)
                    delete section[k+"_comment"];
            }
        }
    }
}

pbxProject.prototype.removeFromPbxFileReferenceSection = function (file, omitExtraQuotes) {
    var i;
...
pbxProject.prototype.removePbxGroupByName = function(name) {
    var k, groups = this.hash.project.objects['PBXGroup'], hash = null;

    for(k in groups)
    {
        if(COMMENT_KEY.test(k))
        {
            if(groups[k]===name)
                hash = k.split(COMMENT_KEY)[0];
        }
    }
...
pbxProject.prototype.pbxVariantGroupsByName = function(name) {
    return this.getObjectsByName(this.hash.project.objects['PBXVariantGroup'], name);
}

pbxProject.prototype.getObjectsByName = function(objects, name) {
    var key, res = [];
    ...
    for (key in objects) {
        ...
        if (objects[key] == name)
            res.push(objects[key.split(COMMENT_KEY)[0]]);
    }
